...
void run_matmul(MM_kernel_params &params, cudaStream_t stream){
  // bf16* C = (bf16 *)malloc(sizeof(bf16) * params.M * params.K);
  // bf16* A = (bf16 *)malloc(sizeof(bf16) * params.M * params.N);
  // bf16* B = (bf16 *)malloc(sizeof(bf16) * params.N * params.K);
  // cudaMemcpy(C, (bf16*)params.C_ptr, sizeof(bf16) * params.M * params.K, cudaMemcpyDeviceToHost);
  // cudaMemcpy(A, (bf16*)params.A_ptr, sizeof(bf16) * params.M * params.N, cudaMemcpyDeviceToHost);
  // cudaMemcpy(B, (bf16*)params.B_ptr, sizeof(bf16) * params.N * params.K, cudaMemcpyDeviceToHost);
  // std::cerr << "C[0] = " << __bfloat162float(C[0]) << std::endl;
  // std::cerr << "A[0] = " << __bfloat162float(A[0]) << std::endl;
